...
// Clean up jobs older than 24h on startup
...
app.get("/api/report-data", async (req, res) => {
  ...
  if (!req.isAuthenticated || !req.isAuthenticated()) {
    return res.status(401).json({ ok: false, error: "Not authenticated. Please refresh the page and log in again." });
  }
  ...
  try {
    ...
    function parseRunFolder(name) {
      ...
      return {
        ...
        label: `${match[3]}/${match[2]}/${match[1]}  ${match[4]}:${match[5]}:${match[6]} IST`,
      };
    }
