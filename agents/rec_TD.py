...
# DEFAULT_INPUT_DIR = Path(r"C:\Users\rohit.rajbhar\Desktop\R\agentpi-clients\inc5\PAY_REC_ALL_VS\recevable\nov")
...
#     s = df[partner_col].astype(str).fillna("").str.replace("\u00A0", " ", regex=False).str.strip()
...
#     Works even if you wrote:  NAME = "Value";
#     """
#     if v is None:
#         return default
#     s = str(v).strip()
...
#         pay_b = pd.to_numeric(row.get(b_pay, ""), errors="coerce")
#         rei_b = pd.to_numeric(row.get(b_rei, ""), errors="coerce")
...
#         pay_p = pd.to_numeric(row.get(p_pay, ""), errors="coerce")
#         rei_p = pd.to_numeric(row.get(p_rei, ""), errors="coerce")
...
#         if not ean:
#             continue
#         rate = pd.to_numeric(row.get(r_col, ""), errors="coerce")
...
#     payable_col = col_map.get("payable", None)
#     dncn_col = col_map.get("dn/cn", None)
...
# # -------------------- 5) COGS --------------------
# EAN_CANDIDATES = ["eancode", "ean code", "ean"]
...
#     rate_num = pd.to_numeric(rate, errors="coerce").fillna(0)
...
#         # add Year/Month into both (so Final_Rec has these cols for upsert logic)
